...
# binmode(STDOUT, ":utf8");
my $admin = $ENV{'PERLBRASILBOT_ADMIN'} || 'HelioLoureiro';
my $token = $ENV{'TELEGRAMBOTTOKEN'};


my ($help, $comandos, $about);
...
if ($comandos) {
  print "Pra ser colado no \@BotFather\n\n"
    ...
    . "quemsoueu - informções da pessoa\n"
    . "knock - quem é?\n"
    ...
    . "uname - saida do comando POSIX\n";
  exit(0);
}

if (!defined($token)) {
  die "Faltando configurações do token de autenticação.";
}
...
my $commands = {
  ...
  "quemsoueu" => sub {
    sprintf "Oi %s, eu sou %s! Como você está?", shift->{from}{username},
      $botname;
  },
  ...
  "telefone" => sub {
    +{
      ...
      reply_markup => {
        keyboard =>
          [[{text => "Belê!", request_contact => \1}, "Não, sai fora!"]],
        one_time_keyboard => \1
      }
    };
  },
  ...
  "ultimafoto" => sub {
    ...
    +{method => "sendPhoto", photo => $pic_id, caption => "Olha ela aqui!"};
  },
  "uptime" => sub {
...
my $message_types = {
  ...
  "contact" => sub {
    ...
    +{
      ...
      parse_mode => "Markdown",
      text       => sprintf(
        "Aqui est�o as informa��es de contacto.\n"
          . "- Nome: *%s*\n- Sobrenome: *%s*\n"
          . "- N�mber de telefone: *%s*\n- Telegram UID: *%s*",
...
#  Pass to the subroutine the message object, and the parameters passed to the cmd.
      ...
      next unless $res;
      my $method
        = ref $res && $res->{method} ? delete $res->{method} : "sendMessage";
      print "Method: $method\n";
      print "res: $res\n";
    ...
    for my $type (keys %{$u->{message} || {}}) {
      next
        ...
        and ref(my $res2 = $message_types->{$type}->($u->{message}));

      my $method2 = delete($res2->{method}) || "sendMessage";

      eval {
        print "Enviando:" . $api->$method2 . "\n";
        $api->$method2({chat_id => $u->{message}{chat}{id}, %$res2});
      } or do {
